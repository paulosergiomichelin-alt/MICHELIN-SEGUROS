...
function getPasswordStrength(password: string): { score: number; label: string; color: string } {
  ...
  if (score <= 2) return { score, label: 'Fraca', color: '#C0392B' };
  if (score <= 4) return { score, label: 'Moderada', color: '#B8860B' };
  return { score, label: 'Forte', color: '#1F8A4C' };
}
    ...
    <div className="space-y-1 w-full group">
      ...
      <div className="relative">
        <input
          {...props}
          className={cn(
            'w-full h-10 bg-white border rounded-lg px-3.5 text-[12px] font-medium text-slate-800 transition-all duration-200 focus:ring-2 hover:border-slate-300 placeholder:text-slate-300 outline-none',
            error
              ? 'border-[#C0392B]/50 focus:ring-[#C0392B]/20 focus:border-[#C0392B]/60'
              : 'border-slate-200 focus:ring-[#1B4D8F]/15 focus:border-[#1B4D8F]/60',
            Icon && 'pl-10',
        ...
        />
        {Icon && (
          <Icon className="absolute left-3.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-400 group-focus-within:text-[#1B4D8F]/70 transition-colors pointer-events-none" />
        )}
      </div>
      {error && (
        <p className="text-[10px] text-[#C0392B] flex items-center gap-1 ml-0.5">
          <AlertCircle className="w-3 h-3 flex-shrink-0" />
          {error}
        </p>
      )}
      {hint && !error && (
        <p className="text-[10px] text-slate-500 flex items-center gap-1 ml-0.5">
    ...
    <div className="space-y-1 w-full group">
      ...
      <div className="relative">
        <select
          {...props}
          className={cn(
            'w-full h-10 bg-white border border-slate-200 rounded-lg px-3.5 text-[12px] font-medium text-slate-800 transition-all duration-200 focus:ring-2 focus:ring-[#1B4D8F]/15 focus:border-[#1B4D8F]/60 focus:outline-none hover:border-slate-300 appearance-none cursor-pointer',
            Icon && 'pl-10',
        ...
        {Icon && (
          <Icon className="absolute left-3.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-400 pointer-events-none" />
        )}
...
function StepResponsavel({
  ...
  const passwordField = (
    <div className="space-y-1 w-full group">
      <label className="text-[8.5px] font-bold text-slate-500 uppercase tracking-[0.18em] ml-0.5 group-focus-within:text-[#1B4D8F] transition-colors">
        Senha <span className="text-[#C0392B]">*</span>
      </label>
      <div className="relative">
        <Lock className="absolute left-3.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-400 group-focus-within:text-[#1B4D8F]/70 transition-colors pointer-events-none" />
        <input type={showSenha ? 'text' : 'password'} value={data.owner_senha}
          onChange={(e) => onChange('owner_senha', e.target.value)}
          placeholder="Mínimo 8 caracteres"
          className={cn('w-full h-10 bg-white border rounded-lg pl-10 pr-10 text-[12px] font-medium text-slate-800 transition-all duration-200 focus:ring-2 hover:border-slate-300 placeholder:text-slate-300 outline-none',
            errors.owner_senha ? 'border-[#C0392B]/50 focus:ring-[#C0392B]/20 focus:border-[#C0392B]/60' : 'border-slate-200 focus:ring-[#1B4D8F]/15 focus:border-[#1B4D8F]/60')} />
  ...
  const confirmField = (
    <div className="space-y-1 w-full group">
      <label className="text-[8.5px] font-bold text-slate-500 uppercase tracking-[0.18em] ml-0.5 group-focus-within:text-[#1B4D8F] transition-colors">
        Confirmar Senha <span className="text-[#C0392B]">*</span>
      </label>
      <div className="relative">
        ...
        <input type={showConfirmar ? 'text' : 'password'} value={data.confirmar_senha}
          ...
          className={cn('w-full h-10 bg-white border rounded-lg pl-10 pr-10 text-[12px] font-medium text-slate-800 transition-all duration-200 focus:ring-2 hover:border-slate-300 placeholder:text-slate-300 outline-none',
            errors.confirmar_senha ? 'border-[#C0392B]/50 focus:ring-[#C0392B]/20 focus:border-[#C0392B]/60' : 'border-slate-200 focus:ring-[#1B4D8F]/15 focus:border-[#1B4D8F]/60')} />
      ...
      {data.confirmar_senha && data.owner_senha === data.confirmar_senha && !errors.confirmar_senha && (
        <p className="text-[10px] text-[#1F8A4C] flex items-center gap-1 ml-0.5">
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-2">
        <p className="text-[8.5px] font-bold text-slate-500 uppercase tracking-[0.18em] ml-0.5">
          Plano <span className="text-[#C0392B]">*</span>
        </p>
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  isSelected
                    ? 'border-gold-deep/40 bg-gold-deep/[0.05] ring-1 ring-gold-deep/20'
                    : 'border-slate-200 bg-white hover:border-slate-300 shadow-sm',
                )}
              >
                {isSelected && (
                  <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-gold-deep/40 to-transparent" />
                )}
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-2">
                      <span className="text-[13px] font-bold text-slate-800">{plan.label}</span>
                      <span
                    ...
                    <p className="text-[11px] text-slate-500 mt-0.5">{plan.desc}</p>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden">
        <div className="px-4 py-3 border-b border-slate-100 bg-gradient-to-r from-gold-deep/[0.06] to-transparent">
          <h4 className="text-[10px] font-black uppercase tracking-[0.20em] text-gold-deep">
            Resumo do Cadastro
          </h4>
        </div>
      ...
      {/* Trial info */}
      <div className="flex items-start gap-3 p-3 rounded-xl bg-gold-deep/[0.08] border border-gold-deep/25">
        <Info className="w-4 h-4 text-gold-deep mt-0.5 flex-shrink-0" />
        <p className="text-[11px] text-slate-700 leading-relaxed">
          Você terá <strong className="text-gold-deep">14 dias de trial gratuito</strong> após o cadastro.
  ...
  return (
    <div className="flex items-center gap-3">
      <div className={cn(
        ...
        highlight ? 'bg-gold-deep/10' : 'bg-slate-100',
      )}>
        <Icon className={cn('w-3 h-3', highlight ? 'text-gold-deep' : 'text-slate-400')} />
      </div>
      <div className="flex-1 min-w-0 flex items-baseline justify-between gap-2">
        ...
        <span
          className={cn(
            ...
            highlight ? 'text-gold-deep' : 'text-slate-700',
          )}
        >
          {value}
        </span>
        ...
        {toasts.map((toast) => (
          <motion.div
            ...
            className={cn(
              ...
              toast.type === 'success' && 'bg-[#E4F5EA] border-[#1F8A4C]/25 text-[#1F8A4C]',
              toast.type === 'error' && 'bg-[#FDE4E4] border-[#C0392B]/25 text-[#C0392B]',
              toast.type === 'info' && 'bg-white border-gold-deep/25 text-gold-deep',
            )}
          >
...
export function CompanyRegistration({ onBack, onSuccess, pageMode = false }: CompanyRegistrationProps = {}) {
  ...
  const cardContent = (
    <motion.div
      ...
      className="w-full max-w-lg relative rounded-[20px] border border-slate-200 bg-white shadow-2xl overflow-hidden"
    >
      <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-gold-deep/40 to-transparent" />
      ...
      <div className="h-1 bg-slate-100 relative">
        <motion.div
          className="absolute inset-y-0 left-0 bg-gradient-to-r from-gold-deep/80 to-gold-deep"
          initial={{ width: 0 }}
          ...
          return (
            <div key={i} className="flex items-center gap-2">
              <div className={cn(
                ...
                isPast && 'bg-[#E4F5EA] border border-[#1F8A4C]/30',
                isCurrent && 'bg-gold-deep/10 border border-gold-deep/30',
                !isPast && !isCurrent && 'bg-slate-100 border border-slate-200',
              )}>
                {isPast ? (
                  <CheckCircle2 className="w-3.5 h-3.5 text-[#1F8A4C]" />
                ) : (
                  <StepIcon className={cn('w-3.5 h-3.5', isCurrent ? 'text-gold-deep' : 'text-slate-300')} />
                )}
              </div>
              <span className={cn(
                'text-[9px] font-bold uppercase tracking-[0.15em] hidden sm:block',
                isCurrent ? 'text-gold-deep' : isPast ? 'text-[#1F8A4C]/80' : 'text-slate-400',
              )}>
                {s.label}
              </span>
              {i < STEPS.length - 1 && (
                <ChevronRight className="w-3 h-3 text-slate-200 mx-1" />
              )}
            </div>
          );
        })}
      ...
      <div className="px-6 pb-6 flex items-center gap-3">
        {step > 0 && (
          <button type="button" onClick={goBack} disabled={isSubmitting}
            className="flex items-center gap-2 h-10 px-4 rounded-lg border border-slate-200 bg-white text-[12px] font-semibold text-slate-500 hover:border-slate-300 hover:text-slate-800 transition-all duration-200 disabled:opacity-50">
            <ChevronLeft className="w-4 h-4" /> Voltar
          </button>
        )}
        <button type="button" onClick={step < STEPS.length - 1 ? goNext : handleSubmit}
          disabled={isSubmitting}
          className="flex-1 flex items-center justify-center gap-2 h-10 rounded-lg bg-[#1B4D8F] text-[12px] font-bold text-white hover:bg-[#153E73] transition-all duration-200 shadow-sm shadow-[#1B4D8F]/20 disabled:opacity-60 disabled:cursor-not-allowed">
          {isSubmitting ? (
      ...
      <div className="pb-4 text-center">
        <p className="text-[9.5px] text-slate-400">Etapa {step + 1} de {STEPS.length}</p>
      </div>
    </motion.div>
  );
  ...
  if (isSuccess) {
    const successContent = (
      ...
      >
        <motion.div
          ...
          className="w-20 h-20 mx-auto rounded-full bg-[#E4F5EA] border border-[#1F8A4C]/25 flex items-center justify-center"
        >
          <CheckCircle2 className="w-10 h-10 text-[#1F8A4C]" />
        </motion.div>
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}>
          ...
          <p className="text-slate-500 text-[13px]">
            <span className="text-slate-800 font-semibold">{formData.razao_social}</span> foi cadastrada com sucesso.
            <br />
            Você tem <strong className="text-gold-deep">14 dias de trial gratuito</strong>.
          </p>
        </motion.div>
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.7 }}
          className="flex items-center justify-center gap-2 text-[11px] text-slate-400">
    ...
    if (pageMode) {
      return (
        <div className="flex flex-col h-full bg-slate-50 text-slate-800 overflow-hidden items-center justify-center">
          {successContent}
  ...
  if (pageMode) {
    ...
    return (
      <div className="flex flex-col h-full bg-slate-50 text-slate-800 overflow-hidden">
        {/* Fixed header */}
        <div className="shrink-0 h-16 border-b border-slate-200 bg-white px-6 md:px-8 flex items-center justify-between shadow-sm z-50">
          <div className="flex items-center gap-3">
            {onBack && (
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <h1 className="text-[13px] font-black uppercase tracking-widest text-slate-900 leading-tight">Nova Empresa</h1>
                <p className="text-[9px] text-slate-500 leading-tight">Preencha os dados para cadastrar</p>
              </div>
            </div>
          </div>
          ...
          <div className="p-6 md:p-10 space-y-10 pb-10">
            ...
            <section>
              <div className="flex items-center gap-3 mb-6 pb-4 border-b border-slate-200">
                <div className="w-8 h-8 rounded-lg bg-gold-deep/10 border border-gold-deep/25 flex items-center justify-center shrink-0">
                  <Building2 className="w-4 h-4 text-gold-deep" />
                </div>
                <div>
                  <h2 className="text-[15px] font-black text-slate-900 tracking-tight">Dados da Empresa</h2>
                  <p className="text-[11px] text-slate-500">Informações cadastrais da empresa</p>
                </div>
              </div>
            ...
            <section>
              <div className="flex items-center gap-3 mb-6 pb-4 border-b border-slate-200">
                <div className="w-8 h-8 rounded-lg bg-[#1B4D8F]/10 border border-[#1B4D8F]/25 flex items-center justify-center shrink-0">
                  <User className="w-4 h-4 text-[#1B4D8F]" />
                </div>
                <div>
                  <h2 className="text-[15px] font-black text-slate-900 tracking-tight">Responsável</h2>
                  <p className="text-[11px] text-slate-500">Proprietário e credenciais de acesso</p>
                </div>
              </div>
            ...
            <section>
              <div className="flex items-center gap-3 mb-6 pb-4 border-b border-slate-200">
                <div className="w-8 h-8 rounded-lg bg-[#E4F5EA] border border-[#1F8A4C]/25 flex items-center justify-center shrink-0">
                  <Settings className="w-4 h-4 text-[#1F8A4C]" />
                </div>
                <div>
                  <h2 className="text-[15px] font-black text-slate-900 tracking-tight">Plano e Configuração</h2>
                  <p className="text-[11px] text-slate-500">Escolha o plano, fuso horário e idioma</p>
                </div>
              </div>
        ...
        {/* Fixed footer */}
        <div className="shrink-0 border-t border-slate-200 bg-white px-6 md:px-10 py-4 flex items-center justify-end gap-3">
          {onBack && (
            <button type="button" onClick={onBack} disabled={isSubmitting}
              className="flex items-center gap-2 h-10 px-5 rounded-xl border border-slate-200 bg-white text-[12px] font-semibold text-slate-500 hover:border-slate-300 hover:text-slate-800 transition-all disabled:opacity-50">
              Cancelar
            </button>
          )}
          <button type="button" onClick={handleSubmitAll} disabled={isSubmitting}
            className="flex items-center gap-2 h-10 px-8 rounded-xl bg-[#1B4D8F] text-[12px] font-bold text-white hover:bg-[#153E73] transition-all shadow-sm shadow-[#1B4D8F]/20 disabled:opacity-60 disabled:cursor-not-allowed">
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-start pt-8 pb-16 px-4">
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-8">
        <div className="w-12 h-12 mx-auto mb-4 rounded-xl bg-gold-deep/10 border border-gold-deep/25 flex items-center justify-center">
          <Building2 className="w-6 h-6 text-gold-deep" />
        </div>
        <h1 className="text-2xl font-black text-slate-900 tracking-tight">Cadastro de Empresa</h1>
        <p className="text-slate-500 text-[12px] mt-1">Preencha os dados e comece a usar em minutos</p>
        {onBack && (
          <button type="button" onClick={onBack}
            className="mt-4 flex items-center gap-2 mx-auto h-9 px-4 rounded-lg border border-slate-200 bg-white text-[11px] font-bold text-slate-500 hover:border-slate-300 hover:text-slate-800 transition-all duration-200">
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }}
        className="mt-4 flex items-center gap-2 text-[10px] text-slate-400">
        <Shield className="w-3 h-3" />
