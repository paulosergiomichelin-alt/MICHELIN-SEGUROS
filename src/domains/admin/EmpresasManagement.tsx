...
const STATUS_CFG: Record<StatusEmpresa, { label: string; color: string; bg: string; border: string; Icon: React.ElementType }> = {
  trial:        { label:'Trial',        color:'#5B7591', bg:'#F1F5F9',  border:'rgba(15,42,74,0.12)',  Icon:Clock },
  ativo:        { label:'Ativo',        color:'#1F8A4C', bg:'#E4F5EA',  border:'rgba(31,138,76,0.25)',  Icon:CheckCircle2 },
  suspenso:     { label:'Suspenso',     color:'#B8860B', bg:'#FFF3DC',  border:'rgba(184,134,11,0.25)',  Icon:PauseCircle },
  inadimplente: { label:'Inadimplente', color:'#C0392B', bg:'#FDE4E4', border:'rgba(192,57,43,0.25)', Icon:AlertTriangle },
  cancelado:    { label:'Cancelado',    color:'#64748B', bg:'#F1F5F9', border:'rgba(100,116,139,0.25)', Icon:XCircle },
};
const PLAN_COLOR: Record<string, string> = { basico:'#64748B', profissional:'#C08A3E', enterprise:'#1B4D8F' };
...
function SkeletonRow() {
  return (
    <tr className="border-t border-slate-100">
      <td className="px-4 py-3.5"><div className="h-3 rounded-full bg-slate-100 animate-pulse w-3/4" /></td>
      <td className="px-4 py-3.5 hidden sm:table-cell"><div className="h-3 rounded-full bg-slate-100 animate-pulse w-2/3" /></td>
      <td className="px-4 py-3.5 hidden md:table-cell"><div className="h-3 rounded-full bg-slate-100 animate-pulse w-1/2" /></td>
      <td className="px-4 py-3.5"><div className="h-3 rounded-full bg-slate-100 animate-pulse w-1/2" /></td>
      <td className="px-4 py-3.5 hidden lg:table-cell"><div className="h-3 rounded-full bg-slate-100 animate-pulse w-1/3" /></td>
      <td className="px-4 py-3.5 hidden lg:table-cell"><div className="h-3 rounded-full bg-slate-100 animate-pulse w-1/2" /></td>
      <td className="px-4 py-3.5"><div className="h-3 rounded-full bg-slate-100 animate-pulse w-1/4" /></td>
    </tr>
  );
}
    ...
    <AnimatePresence>
      <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <motion.div initial={{opacity:0,scale:.95,y:10}} animate={{opacity:1,scale:1,y:0}} exit={{opacity:0,scale:.95,y:10}}
          className="relative w-full max-w-sm rounded-[20px] border border-slate-200 bg-white p-6 shadow-2xl">
          <button onClick={onCancel} className="absolute top-4 right-4 p-1.5 rounded-lg text-slate-400 hover:text-slate-700 hover:bg-slate-100 transition-colors">
          ...
          <h3 className="text-[14px] font-black text-slate-900 mb-1">Alterar status?</h3>
          <p className="text-[12px] text-slate-500 mb-5">
            Você está prestes a <span className="text-slate-800 font-semibold">{label}</span>{' '}
            a empresa <span className="text-gold-deep font-semibold">{empresa.nomeRazaoSocial}</span>.
          </p>
          <div className="flex gap-3">
            <button onClick={onCancel} disabled={loading}
              className="flex-1 h-9 rounded-lg border border-slate-200 bg-white text-[11px] font-semibold text-slate-500 hover:border-slate-300 hover:text-slate-800 transition-all disabled:opacity-50">
              Cancelar
            </button>
...
function DeleteModal({ empresa, onConfirm, onCancel, loading }: DeleteModalProps) {
  ...
  return (
    <AnimatePresence>
      <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="fixed inset-0 z-50 flex items-center justify-center p-4">
        <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={!loading ? onCancel : undefined} />
        <motion.div initial={{opacity:0,scale:.95,y:10}} animate={{opacity:1,scale:1,y:0}} exit={{opacity:0,scale:.95,y:10}}
          className="relative w-full max-w-sm rounded-[20px] border border-[#C0392B]/20 bg-white p-6 shadow-2xl">
          <button onClick={onCancel} disabled={loading} className="absolute top-4 right-4 p-1.5 rounded-lg text-slate-400 hover:text-slate-700 hover:bg-slate-100 transition-colors">
            <X className="w-3.5 h-3.5" />
          </button>

          <div className="w-12 h-12 rounded-xl bg-[#FDE4E4] border border-[#C0392B]/25 flex items-center justify-center mb-4">
            <Trash2 className="w-5 h-5 text-[#C0392B]" />
          </div>

          <h3 className="text-[14px] font-black text-slate-900 mb-1">Excluir empresa?</h3>
          <p className="text-[12px] text-slate-500 mb-1">
            Esta ação é <span className="text-[#C0392B] font-semibold">irreversível</span>.
            O documento da empresa <span className="text-slate-800 font-semibold">{empresa.nomeRazaoSocial}</span> será excluído.
          </p>
          <p className="text-[11px] text-slate-400 mb-4">
            Os usuários e dados de leads desta empresa <strong className="text-slate-500">não</strong> serão removidos automaticamente.
          </p>

          <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">
            Digite <span className="text-[#C0392B]">{expectedWord}</span> para confirmar
          </label>
          <input
            value={typed}
            onChange={e => setTyped(e.target.value)}
            placeholder={expectedWord}
            className="w-full h-9 bg-white border border-[#C0392B]/30 rounded-lg px-3 text-[12px] font-mono text-slate-800 placeholder:text-slate-300 focus:outline-none focus:border-[#C0392B] mb-4"
          />
          ...
          <div className="flex gap-3">
            ...
            <button onClick={onConfirm} disabled={loading || !canDelete}
              className="flex-1 h-9 rounded-lg bg-[#C0392B] text-white text-[11px] font-black uppercase tracking-wide transition-all disabled:opacity-40 flex items-center justify-center gap-2 hover:bg-[#a53225]">
...
function EditModal({ empresa, onClose, onSaved }: EditModalProps) {
  ...
  const inputCls  = "w-full h-9 bg-white border border-slate-200 rounded-lg px-3 text-[12px] text-slate-800 placeholder:text-slate-300 focus:outline-none focus:border-[#1B4D8F]/60 focus:ring-2 focus:ring-[#1B4D8F]/15 transition-all";
  const selectCls = inputCls + " appearance-none cursor-pointer";
  const labelCls  = "block text-[9px] font-black text-slate-500 uppercase tracking-[0.18em] mb-1.5";
  ...
  return (
    <AnimatePresence>
      <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="fixed inset-0 z-50 flex items-center justify-center p-4">
        <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={!saving ? onClose : undefined} />

        <motion.div initial={{opacity:0,scale:.96,y:16}} animate={{opacity:1,scale:1,y:0}} exit={{opacity:0,scale:.96,y:16}}
          className="relative w-full max-w-xl rounded-[22px] border border-slate-200 bg-white shadow-2xl overflow-hidden flex flex-col max-h-[92vh]"
          onClick={e => e.stopPropagation()}>

          {/* Header */}
          <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 shrink-0">
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-lg bg-gold-deep/10 border border-gold-deep/25 flex items-center justify-center">
                <Pencil className="w-4 h-4 text-gold-deep" />
              </div>
              <div>
                <p className="text-[13px] font-black text-slate-900 leading-tight">Editar Empresa</p>
                <p className="text-[10px] text-slate-400">{empresa.nomeRazaoSocial}</p>
              </div>
            </div>
          ...
          {/* Tabs */}
          <div className="shrink-0 border-b border-slate-100 px-5 flex gap-0 overflow-x-auto">
            {modalTabs.map(t => {
              const Icon = t.Icon;
              return (
          ...
          <div className="overflow-y-auto flex-1 p-5">
              ...
              <div className="space-y-4">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <button type="button" onClick={() => fileRef.current?.click()}
                        className="h-8 px-3 rounded-lg border border-slate-200 bg-white text-[11px] font-semibold text-slate-500 hover:border-gold-deep/40 hover:text-gold-deep transition-all flex items-center gap-1.5">
                        <Upload className="w-3 h-3" /> Alterar logo
                      </button>
                      <p className="text-[9.5px] text-slate-400 mt-1">PNG, JPG ou WebP · máx 2 MB</p>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <Field label="CNPJ" span2>
                    <div className={cn(inputCls, "flex items-center opacity-60 cursor-not-allowed bg-slate-50")}>
              ...
              <div className="space-y-4">
                ...
                <div>
                  <button type="button" onClick={() => setFiscal(f => ({ ...f, simplesNacional: !f.simplesNacional }))}
                    className="flex items-center gap-2 text-[11px] font-semibold text-slate-600 hover:text-slate-800 transition-colors">
                    {fiscal.simplesNacional
                      ? <ToggleRight className="w-5 h-5 text-[#1F8A4C]" />
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <Field label="CEP">
                      <div className="relative">
                        ...
                        {fetchingCep && <Loader2 className="absolute right-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gold-deep animate-spin" />}
                        {!fetchingCep && <MapPin className="absolute right-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-300" />}
                      </div>
                    </Field>
                    <Field label="Número">
                    ...
                    <div>
                      ...
                      <div className="mt-1.5">
                        {fiscal.nfseEnvironment === 'producao'
                          ? <span className="text-[8px] font-black uppercase tracking-widest px-2 py-0.5 rounded bg-[#E4F5EA] text-[#1F8A4C] border border-[#1F8A4C]/20">PRODUÇÃO</span>
                          : <span className="text-[8px] font-black uppercase tracking-widest px-2 py-0.5 rounded bg-[#FFF3DC] text-[#B8860B] border border-[#B8860B]/20">HOMOLOGAÇÃO</span>}
                      </div>
                    </div>
                    <div>
            ...
            {tab === 'certificado' && (
              <div className="space-y-4">
                <div className="rounded-xl border border-gold-deep/20 bg-gold-deep/5 p-4">
                  <p className="text-[10px] font-black text-gold-deep uppercase tracking-widest mb-1">Certificado A1</p>
                  <p className="text-[11px] text-slate-500">Arquivo .pfx ou .p12 com a chave privada da empresa emissora. Utilizado para assinatura digital dos XMLs.</p>
                </div>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-3">
                    <div className="flex-1 h-9 bg-slate-50 border border-slate-200 rounded-lg px-3 flex items-center text-[11px] text-slate-500 truncate">
                      {cert.fileName ?? certFile?.name ?? 'Nenhum arquivo selecionado'}
                    </div>
                    <button type="button" onClick={() => certRef.current?.click()}
                      className="h-9 px-3 rounded-lg border border-slate-200 bg-white text-[11px] font-semibold text-slate-500 hover:border-gold-deep/40 hover:text-gold-deep transition-all flex items-center gap-1.5 shrink-0">
                ...
                <div>
                  ...
                  <p className="text-[9.5px] text-slate-400 mt-1">A senha é armazenada de forma criptografada.</p>
                </div>
            ...
            {tab === 'servicos' && (
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <p className="text-[11px] font-semibold text-slate-500">{services.length} serviço{services.length !== 1 ? 's' : ''} cadastrado{services.length !== 1 ? 's' : ''}</p>
                  <button type="button" onClick={() => setEditSvc({ id: crypto.randomUUID(), ativo: true, aliquotaISS: fiscal.aliquotaISS ?? 2 })}
                    className="h-8 px-3 rounded-lg border border-gold-deep/25 bg-gold-deep/10 text-[10px] font-black text-gold-deep hover:brightness-95 transition-all flex items-center gap-1.5">
                ...
                {/* Service form */}
                {editSvc && (
                  <div className="rounded-xl border border-gold-deep/20 bg-gold-deep/5 p-4 space-y-3">
                    <p className="text-[10px] font-black text-gold-deep uppercase tracking-widest">{editSvc.descricao ? 'Editar Serviço' : 'Novo Serviço'}</p>
                    <div className="grid grid-cols-2 gap-3">
                    ...
                    <div className="flex items-center gap-2">
                      <button type="button" onClick={() => {
                        ...
                        setEditSvc(null);
                      }} className="h-8 px-4 rounded-lg bg-[#1B4D8F] text-white text-[10px] font-black uppercase tracking-wide hover:bg-[#153E73] transition-all">
                        Salvar Serviço
                      </button>
                      <button type="button" onClick={() => setEditSvc(null)}
                        className="h-8 px-3 rounded-lg border border-slate-200 bg-white text-[10px] font-semibold text-slate-500 hover:text-slate-800 transition-all">
                        Cancelar
                      </button>
                    </div>
                  </div>
                ...
                {services.length > 0 && (
                  <div className="rounded-xl border border-slate-200 overflow-hidden">
                    {services.map(svc => (
                      <div key={svc.id} className="flex items-center gap-3 px-4 py-3 border-b border-slate-100 last:border-0 hover:bg-slate-50 transition-colors">
                        <div className="flex-1 min-w-0">
                          <p className="text-[11px] font-semibold text-slate-800 truncate">{svc.descricao}</p>
                          <p className="text-[9.5px] text-slate-400">Item {svc.itemListaServico} · ISS {svc.aliquotaISS}%{svc.cnae ? ` · CNAE ${svc.cnae}` : ''}</p>
                        </div>
                        <button type="button" onClick={() => setServices(s => s.map(sv => sv.id === svc.id ? { ...sv, ativo: !sv.ativo } : sv))}
                          className={cn('text-[9px] font-black uppercase tracking-widest px-2 py-0.5 rounded transition-all',
                            svc.ativo ? 'bg-[#E4F5EA] text-[#1F8A4C] border border-[#1F8A4C]/20' : 'bg-slate-100 text-slate-400 border border-slate-200',
                          )}>
                          {svc.ativo ? 'Ativo' : 'Inativo'}
                        </button>
                        <button type="button" onClick={() => setEditSvc(svc)}
                          className="w-6 h-6 rounded-lg border border-slate-200 flex items-center justify-center text-slate-400 hover:text-[#1B4D8F] hover:border-[#1B4D8F]/30 transition-all">
                          <Pencil className="w-3 h-3" />
                        </button>
                        <button type="button" onClick={() => setServices(s => s.filter(sv => sv.id !== svc.id))}
                          className="w-6 h-6 rounded-lg border border-[#C0392B]/20 bg-[#FDE4E4]/40 flex items-center justify-center text-[#C0392B]/70 hover:text-[#C0392B] hover:border-[#C0392B]/40 transition-all">
                          <X className="w-3 h-3" />
                        </button>
                      </div>
                    ))}
                ...
                {services.length === 0 && !editSvc && (
                  <div className="flex flex-col items-center justify-center py-10 gap-3">
                    ...
                    <p className="text-[11px] text-slate-400">Nenhum serviço cadastrado</p>
                    <p className="text-[10px] text-slate-400 text-center max-w-xs">Exemplo: Corretagem de Seguros · Item 10.05 · ISS 2%</p>
                  </div>
                )}
              </div>
            )}
          ...
          {/* Footer */}
          <div className="px-5 py-4 border-t border-slate-100 shrink-0 flex items-center justify-end gap-3">
            <button onClick={onClose} disabled={saving}
              className="h-9 px-4 rounded-lg border border-slate-200 bg-white text-[11px] font-semibold text-slate-500 hover:border-slate-300 hover:text-slate-800 transition-all disabled:opacity-50">
              Cancelar
            </button>
            <button onClick={handleSave} disabled={saving}
              className="h-9 px-5 rounded-lg bg-[#1B4D8F] text-white text-[11px] font-black uppercase tracking-wide flex items-center gap-2 hover:bg-[#153E73] transition-all disabled:opacity-60 shadow-sm shadow-[#1B4D8F]/20">
              {saving
    ...
    <AnimatePresence>
      <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="fixed inset-0 z-40 flex items-end sm:items-center justify-center p-4">
        ...
        <motion.div initial={{opacity:0,y:30}} animate={{opacity:1,y:0}} exit={{opacity:0,y:30}}
          className="relative w-full max-w-md rounded-[20px] border border-slate-200 bg-white shadow-2xl overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-[12px] font-black text-slate-900">{empresa.nomeRazaoSocial}</p>
                {empresa.nomeFantasia && <p className="text-[10px] text-slate-500">{empresa.nomeFantasia}</p>}
              </div>
            </div>
            <div className="flex items-center gap-1.5">
              <button onClick={onEdit}
                className="h-7 px-3 rounded-lg border border-gold-deep/25 bg-gold-deep/10 text-[10px] font-black text-gold-deep hover:brightness-95 transition-all flex items-center gap-1.5">
          ...
          <div className="p-5 space-y-2.5 max-h-[60vh] overflow-y-auto">
            {rows.map(({ label, value }) => (
              <div key={label} className="flex items-baseline justify-between gap-4">
                <span className="text-[9px] font-bold text-slate-400 uppercase tracking-[0.15em] shrink-0">{label}</span>
...
export function EmpresasManagement() {
  ...
  const SortHeader = ({ field, label }: { field: SortField; label: string }) => (
    <button onClick={() => handleSort(field)} className="inline-flex items-center gap-1 group hover:text-slate-700 transition-colors">
      <span>{label}</span>
      <span className="flex flex-col -space-y-0.5">
        <ChevronUp className={cn('w-2.5 h-2.5', sortField===field&&sortDir==='asc' ? 'text-[#1B4D8F]' : 'text-slate-300 group-hover:text-slate-400')} />
        <ChevronDown className={cn('w-2.5 h-2.5', sortField===field&&sortDir==='desc' ? 'text-[#1B4D8F]' : 'text-slate-300 group-hover:text-slate-400')} />
      </span>
    </button>
  );
      ...
      <motion.div initial={{opacity:0,y:-16}} animate={{opacity:1,y:0}} className="mb-6">
        <div className="flex items-center gap-3 mb-1">
          <div className="w-9 h-9 rounded-xl bg-gold-deep/10 border border-gold-deep/25 flex items-center justify-center">
            <Building2 className="w-4.5 h-4.5 text-gold-deep" style={{width:18,height:18}} />
          </div>
          <h1 className="text-[18px] font-black text-slate-900 tracking-tight">Gestão de Empresas</h1>
        </div>
        <p className="text-[11px] text-slate-500 ml-12">
      ...
      <motion.div initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} transition={{delay:.05}} className="flex flex-wrap items-center gap-3 mb-4">
        <div className="relative w-full sm:flex-1 sm:max-w-sm">
          ...
          <input type="text" placeholder="Buscar por nome ou CNPJ..." value={search} onChange={e => setSearch(e.target.value)}
            className="w-full h-9 bg-white border border-slate-200 rounded-lg pl-9 pr-3.5 text-[12px] font-medium text-slate-800 placeholder:text-slate-300 focus:outline-none focus:border-[#1B4D8F]/60 focus:ring-2 focus:ring-[#1B4D8F]/15 transition-all" />
          {search && (
        ...
        <button onClick={fetchEmpresas} disabled={loading}
          className="h-9 px-3.5 rounded-lg border border-slate-200 bg-white text-slate-500 hover:border-slate-300 hover:text-slate-800 transition-all disabled:opacity-50 flex items-center gap-2 text-[11px] font-semibold">
          <RefreshCw className={cn('w-3.5 h-3.5', loading && 'animate-spin')} /> Atualizar
        </button>
        <button onClick={() => setShowNew(true)}
          className="h-9 px-4 rounded-lg bg-[#1B4D8F] text-white text-[11px] font-black uppercase tracking-wide flex items-center gap-2 hover:bg-[#153E73] transition-all sm:ml-auto shadow-sm shadow-[#1B4D8F]/20">
          <Plus className="w-3.5 h-3.5" /> Nova Empresa
        </button>
      </motion.div>

      {error && (
        <motion.div initial={{opacity:0}} animate={{opacity:1}} className="flex items-center gap-3 p-4 rounded-xl border border-[#C0392B]/20 bg-[#FDE4E4] mb-4">
          <AlertCircle className="w-4 h-4 text-[#C0392B] shrink-0" />
          <p className="text-[12px] text-[#C0392B]">{error}</p>
          <button onClick={fetchEmpresas} className="ml-auto text-[11px] font-semibold text-[#C0392B] hover:text-[#a53225] underline">Tentar novamente</button>
        </motion.div>
      )}
      ...
      <motion.div initial={{opacity:0,y:16}} animate={{opacity:1,y:0}} transition={{delay:.1}}
        ...
        {!loading && !error && filtered.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 gap-4">
            ...
            {search && <button onClick={() => setSearch('')} className="text-[11px] text-gold-deep hover:brightness-90 underline">Limpar busca</button>}
          </div>
        )}
                ...
                <tr className="border-b border-slate-100 bg-slate-50">
                  {[
                    ...
                    <th key={i} className={cn('px-4 py-3', right && 'text-right', cls)}>
                      <span className="text-[9px] font-black uppercase tracking-[0.18em] text-slate-400">{col}</span>
                    </th>
                  ))}
                </tr>
              </thead>
                      ...
                      <motion.tr key={e.id} initial={{opacity:0,y:6}} animate={{opacity:1,y:0}} transition={{delay:idx*.03}}
                        ...
                        <td className="px-4 py-3.5">
                          <div className="flex items-center gap-2.5">
                            <div className="w-7 h-7 rounded-lg bg-gold-deep/10 border border-gold-deep/20 flex items-center justify-center shrink-0 overflow-hidden">
                              {e.logoUrl
                                ? <img src={e.logoUrl} alt={e.nomeRazaoSocial} className="w-full h-full object-contain" />
                                : <Building2 className="w-3.5 h-3.5 text-gold-deep/60" />}
                            </div>
                            <div className="min-w-0">
                              <p className="text-[12px] font-semibold text-slate-800 truncate max-w-[180px]">{e.nomeRazaoSocial}</p>
                              {e.nomeFantasia && <p className="text-[10px] text-slate-400 truncate max-w-[180px]">{e.nomeFantasia}</p>}
                            </div>
                          </div>
                        </td>
                        ...
                        <td className="px-4 py-3.5 hidden sm:table-cell">
                          <span className="text-[11px] font-mono text-slate-500">{formatCnpj(e.cnpj)}</span>
                        </td>
                        ...
                        <td className="px-4 py-3.5 hidden lg:table-cell">
                          {e.status === 'trial' && e.trialExpiraEm ? (
                            <div className="space-y-0.5">
                              <p className={cn('text-[11px] font-semibold', expiring ? 'text-[#C0392B]' : 'text-[#B8860B]')}>
                                {daysLeft !== null ? `${daysLeft}d restantes` : '—'}
                              </p>
                              <p className="text-[9.5px] text-slate-400">expira {formatDate(e.trialExpiraEm)}</p>
                            </div>
                          ) : (
                            <span className="text-[11px] text-slate-400">{formatDate(e.criadoEm)}</span>
                          )}
                        </td>
                        ...
                        <td className="px-4 py-3.5 text-right">
                          <div className="flex items-center justify-end gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button onClick={() => setDetailEmpresa(e)} title="Ver detalhes"
                              className="w-7 h-7 rounded-lg border border-slate-200 bg-white flex items-center justify-center text-slate-400 hover:border-gold-deep/40 hover:text-gold-deep transition-all">
                              <Eye className="w-3.5 h-3.5" />
                            </button>
                            <button onClick={() => setEditEmpresa(e)} title="Editar"
                              className="w-7 h-7 rounded-lg border border-slate-200 bg-white flex items-center justify-center text-slate-400 hover:border-[#1B4D8F]/30 hover:text-[#1B4D8F] transition-all">
                              <Pencil className="w-3.5 h-3.5" />
                            </button>
                            {e.status !== 'ativo' && (
                              <button onClick={() => setConfirmStatus({ empresa:e, next:'ativo' })} title="Ativar"
                                className="w-7 h-7 rounded-lg border border-[#1F8A4C]/20 bg-[#E4F5EA] flex items-center justify-center text-[#1F8A4C]/80 hover:text-[#1F8A4C] hover:border-[#1F8A4C]/40 transition-all">
                                <ShieldCheck className="w-3.5 h-3.5" />
                              </button>
                            )}
                            {e.status !== 'suspenso' && e.status !== 'cancelado' && (
                              <button onClick={() => setConfirmStatus({ empresa:e, next:'suspenso' })} title="Suspender"
                                className="w-7 h-7 rounded-lg border border-[#B8860B]/20 bg-[#FFF3DC] flex items-center justify-center text-[#B8860B]/80 hover:text-[#B8860B] hover:border-[#B8860B]/40 transition-all">
                                <ShieldOff className="w-3.5 h-3.5" />
                              </button>
                            )}
                            <button onClick={() => setDeleteTarget(e)} title="Excluir"
                              className="w-7 h-7 rounded-lg border border-[#C0392B]/20 bg-[#FDE4E4] flex items-center justify-center text-[#C0392B]/80 hover:text-[#C0392B] hover:border-[#C0392B]/40 transition-all">
