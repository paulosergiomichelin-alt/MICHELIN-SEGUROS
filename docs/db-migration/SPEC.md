...
| ADR-9 | `whatsapp_conversations` e `whatsapp_messages` — hoje **declaradas no `COLLECTION_MAP` do `DataService` mas na prática nunca persistidas** (vivem só em `Map` de memória em `_api/lib/conversationCache.ts`, comentário explícito no código: "Avoids Firestore writes entirely... prevents quota exhaustion") — passam a ser **tabelas Postgres reais**. | Isso não é 1:1 (não existe dado Firestore pra migrar aqui), mas é a correção natural de uma inconsistência já documentada nos commits recentes ("fragilidades e vazamentos de memória", "corrigir paginação findMessages") — dado em memória de processo não sobrevive a reinício/instância diferente em serverless. Ver Risco 3 na seção 8; **o usuário deve confirmar essa decisão antes da Fase 4** (é a única mudança de comportamento, não só de motor de banco). |
...
| 3 | Promover `whatsapp_conversations`/`whatsapp_messages`/e-mails de "memória" para "tabela real" (ADR-9) é uma mudança de comportamento, não só de motor de banco. | Fica marcado como decisão a confirmar com o usuário antes da Fase 4, não é assumido silenciosamente. |
